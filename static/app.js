...
class TimeGutter {
    ...
    tryGetCellHeight() {
        ...
        try { this.cellHeight = this.term._core._renderService.dimensions.actualCellHeight; } catch(e) {}
...
function applyTheme(themeData) {
    ...
    if (term) {
        term.setOption('theme', {
            ...
            black: themeData.Black, red: themeData.Red, green: themeData.Green, yellow: themeData.Yellow, blue: themeData.Blue, magenta: themeData.Magenta, cyan: themeData.Cyan, white: themeData.White,
            brightBlack: themeData.BrightBlack, brightRed: themeData.BrightRed, brightGreen: themeData.BrightGreen, brightYellow: themeData.BrightYellow, brightBlue: themeData.BrightBlue, brightMagenta: themeData.BrightMagenta, brightCyan: themeData.BrightCyan, brightWhite: themeData.BrightWhite
        });
    }
}
...
function handleCustomThemeChange() {
    applyTheme({ Name: "Custom", Background: document.getElementById('theme-bg').value, Foreground: document.getElementById('theme-fg').value, Cursor: document.getElementById('theme-cursor').value, ...themePresets["Default"] });
    saveConfigSilently();
}
...
function listScripts() {
    fetch('/listscripts', { method: 'POST' }).then(r => r.json()).then(data => {
        if (data.status === 'ok') {
            ...
            data.scripts.forEach(s => { const opt = document.createElement('option'); opt.value = s; opt.textContent = s; select.appendChild(opt); });
        }
    });
}
...
function runScript() {
    ...
    fetch('/runscript', { method: 'POST', body: formData }).then(r => r.json()).then(data => {
        if (data.status !== 'ok') { appendToScriptConsole("❌ Failed to start: " + data.message); isScriptRunning = false; document.getElementById('script-status-text').textContent = 'Error'; document.getElementById('script-status-text').style.color = 'var(--danger-color)'; }
...
function stopScript() {
    fetch('/stopscript', { method: 'POST' }).then(r => r.json()).then(data => {
        if (data.status === 'ok') { isScriptRunning = false; document.getElementById('script-status-text').textContent = 'Stopped'; document.getElementById('script-status-text').style.color = 'var(--warning-color)'; }
    });
}

function appendToScriptConsole(msg) {
    const con = document.getElementById('script-console');
    const div = document.createElement('div'); div.textContent = msg; con.appendChild(div); con.scrollTop = con.scrollHeight;
...
function scanPorts() {
    ...
    fetch('/scan', { method: 'POST' }).then(r => r.json()).then(data => {
        if (data.status === 'ok') showPortSelection(data.ports); else showPortSelectionError(data.message);
...
function setMode(mode) {
    ...
    fetch('/setmode', { method: 'POST', body: formData }).then(r => r.json()).then(data => { if (data.status === 'ok') { currentMode = mode; updateMode(mode); } });
}
...
function setShellEnabled(enabled, shouldSave = true) {
    ...
    if (enabled) {
        ...
        if (!term) {
            try { initTerminal(); } catch (e) { syncShellUI(false); shellEnabled = false; updateShellStatus(false); addToOutput('[System] Failed to initialize terminal: ' + e.message); userActionLock = false; return; }
...
function initTerminal() {
    ...
    timeGutter = new TimeGutter(timeGutterEl);
    timeGutter.bindTerminal(term);
    updateGutterVisibility();
    ...
    setTimeout(() => {
        ...
        if (document.activeElement === document.body || document.activeElement === null || document.activeElement.tagName === 'DIV') term.focus();
    }, 200);
}
...
function applyShellHighlight(text) {
    ...
    return text.replace(combinedHighlightRegex, function(match, ...groups) { for (let i = 0; i < highlightColorMap.length; i++) { if (groups[i] !== undefined) return hexToAnsi(highlightColorMap[i]) + match + '\x1b[0m'; } return match; });
}

function applyHighlight(text) {
    let result = text.replace(/[&<>"]/g, tag => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'})[tag] || tag);
    if (combinedHighlightRegex) {
        result = result.replace(combinedHighlightRegex, function(match, ...groups) { for (let i = 0; i < highlightColorMap.length; i++) { if (groups[i] !== undefined) return `<span style="color:${highlightColorMap[i]};font-weight:bold;">${match}</span>`; } return match; });
    }
    return result;
}
...
function exportLog() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `serial_log_${new Date().toISOString().replace(/[:.]/g, '-')}.txt`; a.click(); URL.revokeObjectURL(url);
}
...
function getFormData() {
    ...
    if (themeName === "Custom") { themeData.Background = document.getElementById('theme-bg').value; themeData.Foreground = document.getElementById('theme-fg').value; themeData.Cursor = document.getElementById('theme-cursor').value; Object.assign(themeData, themePresets["Default"]); }
