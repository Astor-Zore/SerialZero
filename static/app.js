...
class TimeGutter { 
    constructor(element) { 
        this.el = element; 
        this.markers = []; // Use xterm native markers to track logical lines 
        this.lineTimestamps = []; // Fallback array if markers fail 
        this.cellHeight = 0; 
        this.term = null; 
        this.viewport = null; 
    ...
    bindTerminal(term) { 
        ...
        term.onLineFeed(() => { 
            ...
            // Find the logical start of this completed line 
            ...
            if (startLine && !startLine.isWrapped) { 
                ...
                // Try using registerMarker (xterm v5+) or addMarker (older versions) 
                try { 
                ...
                if (marker) { 
                    // Success: use marker (survives resize) 
                    ...
                    // Fallback: use array (might misalign on resize, but guarantees display) 
    ...
    render() { 
        ...
        // If markers are available, use them (perfect resize handling) 
...
function applyTheme(themeData) { 
    ...
    if (term) { 
        term.setOption('theme', { 
            ...
            selectionBackground: themeData.Cursor + '40', 
...
function updateHighlightRules(lines) { 
    let rules = lines.map(line => { 
        ...
        try { 
            return { regexStr: parts[0], color: parts.slice(1).join(':') }; 
        } catch (e) { 
            return null; 
        } 
    }).filter(r => r !== null); 
...
function exportLog() { 
    ...
    if (shellEnabled && term) { 
        ...
        // Build a map of absolute line index to timestamp 
        const tsMap = {}; 
        if (hasTimestamps) { 
            if (timeGutter.markers && timeGutter.markers.length > 0) { 
                timeGutter.markers.forEach(m => { 
                    if (!m.marker.isDisposed) { 
                        tsMap[m.marker.line] = m.ts; 
                    } 
                }); 
            } else if (timeGutter.lineTimestamps) { 
                timeGutter.lineTimestamps.forEach((ts, index) => { 
                    if (ts) tsMap[index] = ts; 
                }); 
            } 
        } 
        ...
        for (let i = 0; i < buffer.length; i++) { 
            ...
            if (line) { 
                ...
                // If the line is wrapped, merge it with the previous logical line 
                if (line.isWrapped && mergedLines.length > 0) { 
                    mergedLines[mergedLines.length - 1].text += lineText; 
                } else { 
                    let ts = hasTimestamps ? (tsMap[i] || '') : ''; 
                    mergedLines.push({ text: lineText, ts: ts }); 
                } 
            } 
        } 
...
function updatePortInfo(port, baud) { 
    document.getElementById('port-info').textContent = `${port} | ${baud}`; 
} 
...
function getFormData() { 
    ...
    if (themeName === "Custom") { 
        ...
        Object.assign(themeData, themePresets["Default"]); 
    } else { 
    ...
    return { 
        ...
        UI: { 
            ...
            FontSize: parseInt(document.getElementById('fontsize-input').value), 
            Timestamp: timestampEnabled, 
            Shell: shellEnabled, 
            Scrollback: parseInt(document.getElementById('scrollback-input').value) || 100000 
        }, 
        Highlight: { 
            Groups: document.getElementById('highlight-input').value.split('\n').map(l=>l.trim()).filter(l=>l) 
        }, 
        Log: { Path: './logs' }, 
        Theme: themeData 
    }; 
...
function showPortSelection(ports) { 
    const modalBody = document.getElementById('modal-body'); 
    if (ports.length === 0) { 
        modalBody.innerHTML = `<p>No available ports.</p>`; 
        return; 
    } 
...
function showPortSelectionError(msg) { 
    document.getElementById('modal-body').innerHTML = `<p style="color:red;">Error: ${msg}</p>`; 
} 
